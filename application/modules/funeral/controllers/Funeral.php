	...
	public function funeral_list_ajax($process_action='View') { // รายการสงเคราะห์ผู้สูงอายุในภาวะยากลำบาก
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
			...
			foreach ($list as $i=>$funeral) {
				...
				$org_Update_id = $funeral->update_org_id;
				if($org_id =='174'){
					if($org_Update_id != null && $org_Update_id != ''){
						$orgU = $this->difficult_model->getOrg($org_Update_id);
						...
						$org2 = $this->difficult_model->getOrg($org_id);
...
/*			$data['fnrl_info'] = $this->common_model->custom_query("
  		...
  		set_js_asset_footer('mapmarker.js','webconfig'); //Set JS mapmarker.js
			...
			if($process_action=='Add' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				if($frm->run($this)){//Valid Data
					...
					$info_insert['tel_no_home']	= get_inpost('fnrl_info[req_tel_home]');
					$info_insert['tel_no']	= get_inpost('fnrl_info[req_tel_no_mobile]');
					$info_insert['email_addr']	= get_inpost('fnrl_info[req_email_addr]');					
					$info_insert['bank_acc_no']	= get_inpost('fnrl_info[req_bank_acc_no]');
					$info_insert['bank_name']	= get_inpost('fnrl_info[req_bank_name]');
					...
					$info_insert2['tel_no']	= get_inpost('pers_info[tel_no]');
					...
					$info_insert2['pren_code']	= get_inpost('impv_info2[pren_code]');				
					unset($info_insert2['pren_name']);
					...
					$info_insert3['tel_no']	= get_inpost('fnrl_info[req_pers_aprv_tel_no_mobile]');						
					$info_insert3['pren_code']	= get_inpost('impv_info3[pren_code]');				
					unset($info_insert3['pren_name']);						
					...
					$info_insert5['tel_no']	= get_inpost('fnrl_info[not_survey_aprv_tel_no_mobile]');
					$info_insert5['pren_code']	= get_inpost('impv_info5[pren_code]');				
					unset($info_insert5['pren_name']);	
				...
				if(isset($row['fnrl_id'])) {
					...
					$data['req_prename2'] = $this->common_model->getPrename($data['req_diff_info']['pren_code']);
					$data['req_prename3'] = $this->common_model->getPrename($data['req_diff_info4']['pren_code']);
					$data['req_prename4'] = $this->common_model->getPrename($data['req_diff_info5']['pren_code']);
						// print_r($data['req_prename']);exit();
	...
	public function getDataDead() { 
		...
		$rs = $this->common_model->getDead($input['req_id']);
