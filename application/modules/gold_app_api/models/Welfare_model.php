...
class Welfare_model extends CI_Model
{
    ...
    // ตรวจสอบการเสียชีวิต
    public function check_fnrl_status($pers_id){
        $mysql_query = "select *
                        from fnrl_info 
                        where delete_datetime is null
                        and pers_id = '".$pers_id."'  order by fnrl_id desc" ;
        $query = $this->db->query($mysql_query);
        $fnrl = $query->row();
        if($fnrl != null)
            if(empty($fnrl->cond_status) || $fnrl->cond_status == null){
                return 1;
            }else if($fnrl->cond_status == 'อนุมัติ'){
                return 2;
            }else{
                return 3;
            }
        else
            return 0; // ยังมีชีวิตอยู่
    }
    ...
    #region check status
    // ตารางการสงเคราะห์ผู้สูงอายุในภาวะยากลำบาก
    public function check_diff_status($pers_id){
        $mysql_query_count = "select count(*) as count
                              from diff_info 
                              where delete_datetime is null
                              and payment_status = 'อนุมัติ'
                              and YEAR(date_of_req) = YEAR(CURRENT_DATE())
                              and pers_id = '".$pers_id."'" ;
        $query_count = $this->db->query($mysql_query_count);
        $diff_count = $query_count->row();
        if(($diff_count->count) > 3){
            return 4;
        }
        $mysql_query = "select *
                        from diff_info 
    ...
    // ตารางการสงเคราะห์ผู้สูงอายุในภาวะยากลำบาก
    public function save_diff($pers_id,$pers_id_reg){
        $data_insert = array();
        $data_insert['pers_id'] = $pers_id;
        $data_insert['req_pers_id'] = $pers_id_reg;
    ...
    // ตารางศูนย์พัฒนาการจัดสวัสดิการสังคมผู้สูงอายุ
    public function save_adm($pers_id,$pers_id_reg){
        $data = array(
            'pers_id' => $pers_id,
            'req_pers_id' => $pers_id_reg,
    ...
    // Save to table fnrl_info
    // ตารางการสงเคราะห์ในการจัดการศพตามประเพณี
    public function save_fnrl($pers_id,$pers_id_reg){
        $data = array(
            'pers_id' => $pers_id,
            'req_pers_id' => $pers_id_reg,
            'date_of_req' => date_format(new DateTime(), 'Y-m-d'),
    ...
    public function check_welfare_status($pers_id ,$date_of_birth){
        ...
        if($this->check_age($date_of_birth)){
            $status["fnrl"] = $this->status_enum($this->check_fnrl_status($pers_id));
    ...
    public function check_welfare_status_default(){
        $status = array(
            "fnrl" => $this->status_enum(-1),
            "diff" => $this->status_enum(-1),
            "impv" => $this->status_enum(-1),
    ...
    public function diff_history($pers_id){
        ...
        $mysql_query = "with rpt as (
                                select d.* 
                                from diff_info d
                                where d.delete_datetime is null
                                and d.pers_id = {$pers_id}
                                order by d.date_of_req desc
                              )
                   select  
                            CONCAT(
                                    (CASE WHEN (sp.prename_th is null or sp.prename_th = '') THEN '' ELSE sp.prename_th END)
                                    ,(CASE WHEN (pi.pers_firstname_th is null or pi.pers_firstname_th = '') THEN '' ELSE pi.pers_firstname_th END)
                                    ,'  '
                                    ,(CASE WHEN (pi.pers_lastname_th is null or pi.pers_lastname_th = '') THEN '' ELSE pi.pers_lastname_th END)
                                ) as fullname_req
                            ,main.date_of_req
                            ,c.chn_name as channel_name
                            ,CASE WHEN (main.cond_status is null or main.cond_status = '') THEN 'รอดำเนิการ' ELSE  main.cond_status END as status
                            ,CASE WHEN (main.cond_remark is null ) THEN '' ELSE  main.cond_remark END as status_remark
                    from rpt main
                    left join std_req_channel as c on c.chn_code = main.chn_code
                    left join pers_info as pi on pi.pers_id = main.req_pers_id
                    left join std_prename as sp on sp.pren_code = pi.pren_code";
        $query = $this->db->query($mysql_query);
        $data = $query->result();
        return $data;
        // $this->db->SELECT("*");
    ...
    public function impv_history($pers_id){
        ...
        $mysql_query = "with rpt as (
                        select d.* 
                        from impv_home_info d
                        where d.delete_datetime is null
                        and d.pers_id = {$pers_id}
                        order by d.date_of_svy desc
                    )
                    select  
                                CONCAT(
                                                (CASE WHEN (sp.prename_th is null or sp.prename_th = '') THEN '' ELSE sp.prename_th END)
                                ...
                                ,main.date_of_svy as date_of_req
                        ,c.chn_name as channel_name
                        ,CASE WHEN (main.consi_result is null or main.consi_result = '') THEN 'รอดำเนิการ' ELSE  main.consi_result END as status
                                ,CASE WHEN (main.consi_result_remark is null ) THEN '' ELSE  main.consi_result_remark END as status_remark
                    from rpt main
                    left join std_req_channel as c on c.chn_code = main.chn_code
                    left join pers_info as pi on pi.pers_id = main.pers_id
                    left join std_prename as sp on sp.pren_code = pi.pren_code";
        $query = $this->db->query($mysql_query);
        $data = $query->result();
        return $data;
    ...
    public function adm_history($pers_id){
        ...
        $mysql_query = "with rpt as (
                        ...
                        select  
                                    ...
                                    ,main.date_of_req
                            ,c.chn_name as channel_name
                            ,CASE WHEN (main.date_of_dis is not null and main.date_of_dis != '') THEN 'จำหน่าย' 
                                        WHEN (main.date_of_adm is null and main.date_of_adm != '') THEN 'ดำเนินการ(รับเข้า)' 
                                        ELSE  'แจ้งความประสงค์' END as status
                                    ,'' as status_remark
                        from rpt main
                        left join std_req_channel as c on c.chn_code = main.chn_code
                        left join pers_info as pi on pi.pers_id = main.req_pers_id
    ...
    public function fntl_history($pers_id){
        ...
        $mysql_query = "with rpt as (
                        ...
                        select  
                                    ...
                                    ,main.date_of_req as date_of_req
                            ,c.chn_name as channel_name
                            ,CASE WHEN (main.cond_status is null or main.cond_status = '') THEN 'รอดำเนิการ' ELSE  main.cond_status END as status
                                    ,CASE WHEN (main.cond_remark is null ) THEN '' ELSE  main.cond_remark END as status_remark
                        from rpt main
