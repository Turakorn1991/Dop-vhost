...
class Authentication extends CI_Controller
{
            ...
            if(!empty($id) && !empty($password))
            {
                ...
                if(isset($result)) {
                    ...
                    if(isset($result)) {
                        ...
                        echo json_encode(array('success'=>false,'message'=>'เลขบัตรประจำตัวประชาชนหรือรหัสผ่านไม่ถูกต้อง'));
                    }
                }else{
    ...
    public function get_token_with_go_to_gold_admin(){
        $pid = $this->input->post("pid");
        $userid = $this->input->post("userid");
        $date = new DateTime();
        date_add($date, date_interval_create_from_date_string('3 months'));
        $result = array('userid'=>$userid,'pid'=>$pid ,'expires'=>$date);
        $token = JWT::encode($result, ReadTxt::readkey());
        echo json_encode(array('success'=>true,'token'=>$token));
    }

    public function get_data_with_go_to_gold_admin(){
        $token = $this->input->post("key");
        if(isset($token)){
            $result = JWT::decode($token,ReadTxt::readkey());
            echo json_encode(array('success'=>true,'userid'=>$result->userid ,'pid'=>$result->pid));
        }
        else{
            echo json_encode(array('success'=>false,'userid'=>'' ,'pid'=>''));
        }
    }
