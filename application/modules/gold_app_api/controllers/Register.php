...
class Register extends CI_Controller
{
                        ...
                        try{
                            ...
                            if($pers_info != null){
                                ...
                                $data = array(
                                    ...
                                    'date_of_birth' => Register::convertDateStr($date_of_birth),
                                    'pren_code'  => $pren_code,
                                    'tel_no' => $tel_no,
                                    'insert_user_id' => 18,
                                    'insert_org_id' => 174,
    ...
    public function index() 
    {
        ...
        && isset($reg_laser_id) && isset($reg_date_of_birth) && isset($reg_tel_no) && isset($reg_email)){
            ...
            && !empty($reg_laser_id)&& !empty($reg_date_of_birth)){
                ...
                $pers_user = $this->has_user_for_check($reg_pid);
                if($pers_user == null){
                    try{
                        // ข้อมูลผู้ยื่น
                        $pers_id_reg = 0;
                        ...
                        $pers_info_reg = $this->has_pers_info_for_check($reg_pid);
                        if($pers_info_reg != null){
                            ...
                            $data = array(
                                ...
                                'date_of_birth' => Register::convertDateStr($reg_date_of_birth),
                        ...
                        $pers_id = 0;
                        $pre_addr_id = 0;
                        $pers_info = $this->has_pers_info_for_check($pid);
                        if($pers_info != null){
                            $pers_id = $pers_info->pers_id;
                        ...
                        $data_user_gold = $this->has_user_gold($pers_id_reg,$reg_pid);
    ...
    private function has_pers_info_for_check($pid){
        $sql = "select pers_id , pre_addr_id from pers_info where pid = '".$pid."' limit 1";
        return $this->db->query($sql)->row();
    }

    private function has_user_for_check($pid){
        $sql = "select pers_id from app_user where pid = '".$pid."' limit 1";
        return $this->db->query($sql)->row();
    }
    ...
    private static function convertDateStr($dateStr){
