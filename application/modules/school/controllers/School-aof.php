...
class School extends MX_Controller {
	...
	public function uploadPic(){
		...
		if($this->upload->do_upload()){
			...
			$image_path = base_url("uploads/".$info['rw_name'].$info['file_ext']);
			$data['photo_file_name'] = $image_path;
			unset($data['submit']);
			$this->load->model('School_model');
			if($this->School_model->insertImage($data)){
...
/* 			echo '<pre>';
	...
	public function viewImages(){
		$this->load->model('School_model');
		$images = $this->School_model->getImages();
	...
	public function photo2($process_action='Add',$schl_id=0) { //แบบขึ้นทะเบียน (ภาพถ่าย)
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
			...
			if($process_action=='Add' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				//add id variable
				$this->common_model->insert('schl_photo',$data_insert_1); //$id_center_info = 
  ...
  public function del_schl_edu(){
	...
	public function center_list_ajax($process_action='View') { // รายการสงเคราะห์ผู้สูงอายุในภาวะยากลำบาก //รายการ ศพอส
		...
		//	$this->load->model('funeral_list_model','funeral');
			...
			$list = $this->school->get_datatables(); //do new filter : ref. function in School_model.php
			$data = array();
			$no = $_POST['start'];
			// dieArray($list);
			...
			?>  */
				...
				if($center->qlc_id!=''){
					...
					$qlc_select = $this->common_model->query
						...
						and delete_user_id IS NULL AND (delete_datetime IS NULL || delete_datetime = '0000-00-00 00:00:00') 
						order by insert_datetime DESC, insert_id DESC, qlc_id ASC, qlc_kpi_code ASC)")*/
						("SELECT insert_datetime, max(date(insert_datetime)), max(time(insert_datetime))
						FROM qlc_kpi
						WHERE qlc_id = {$center->qlc_id}
						and time(insert_datetime) = 
						(SELECT max(time(insert_datetime))
				...
				if($center->qlc_id!=''){
					$year_of_sponsorship = $this->common_model->query("SELECT (year_of_sponsorship)+543 as year_of_sponsorship FROM qlc_info WHERE qlc_id = {$center->qlc_id} group by qlc_id;")->row_array(); //SELECT year(year_of_sponsorship)+543 as year_of_sponsorship 
