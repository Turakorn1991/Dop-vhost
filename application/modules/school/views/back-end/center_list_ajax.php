...
?>
      <a class="navbar-minimalize minimalize-styl-2 btn btn-primary btn-export center-list-xls-export" style="margin-left: 0px; background-color: #2f4250; border: 0;font-size: 17px; padding: 2px 20px 2px 20px;"
        <?php if (!isset($tmp1['perm_status'])) {?>readonly<?php }?>
        title="<?=$tmp['app_name']?>"
      >
        <i style='font-size:14px;' class="fa fa-table" aria-hidden="true"></i>
        ส่งออกไฟล์
      </a>
      <form class='center-list-xls-form' method='post' action="<?=site_url('report/F10/xls')?>">
        <input filter-col='1' type="hidden" name='columns[1]'>
        <input filter-col='2' type="hidden" name='columns[2]'>
        <input filter-col='3' type="hidden" name='columns[3]'>
        <input filter-col='4' type="hidden" name='columns[4]'>
        <input filter-col='5' type="hidden" name='columns[5]'>
        <input filter-col='6' type="hidden" name='columns[6]'>
        <input filter-col='7' type="hidden" name='columns[7]'>
        <input filter-col='8' type="hidden" name='columns[8]'>
  ...
  <script>
    ...
    $('body').on('click', '.center-list-xls-export', e => {
      $('.column_filter').each((i, ele) => {
        $('form.center-list-xls-form input[filter-col=' + (i + 1) + ']').val($(ele).val())
      });
      $('form.center-list-xls-form:first').submit();
    });
  </script>
    ...
    <table id="dtable" class="table table-striped table-bordered table-hover dataTables-example" style="margin-top: 0px !important; width:100% !important;" >
      <thead style="font-size: 15px;">
        <tr>
            ...
            <th style="width:8% !important;" class="text-center">ภาค</th>
            <th style="width:8% !important;" class="text-center">จังหวัด</th>
            <th class="text-center" style="width:15% !important;" class="text-center">ชื่อหน่วยงาน ศพอส.</th>
            <!--<th style="width:12% !important;" class="text-center">ที่ตั้ง</th>-->
            <!--<th style="width:10% !important;" class="text-center">เบอร์โทรศัพท์</th>-->
            <th style="width:4% !important;" class="text-center">ปีที่ประเมินล่าสุด</th>
            <th style="width:3% !important;" class="text-center">เกรด</th>
            <th style="width:8% !important;" class="text-center">อำเภอ</th>
            <th style="width:4% !important;" class="text-center">ปีที่ได้รับงบประมาณเป็นครั้งแรก</th>
            <!--<th style="width:4% !important;" class="text-center">จำนวนครั้งที่ได้รับงบประมาณ</th>-->
            <th style="width:4% !important;" class="text-center">จำนวนกิจกรรมปีปัจจุบัน</th>
            <th style="width:4% !important;" class="text-center">จำนวนโรงเรียน</th>
