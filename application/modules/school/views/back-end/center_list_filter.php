  ...
  <div class="well" style="background-color: rgb(255, 255, 255);">
     <div class="container-fluid">
        ...
        <div class="form-group row">
          ...
          <div class="col-xs-12 col-sm-6">
            <input data-column="1" type="text" class="form-control column_filter" id="col1_filter" placeholder="ชื่อหน่วยงาน ศพอส."> <!-- do new filter : data-column="1", id="col1_filter" -->
          </div>
        </div>
...
foreach ($temp as $key => $row) {?>
          ...
          <div class="col-xs-12 col-sm-3" style="padding-top: 12px;">
              ...
              <select  title="จังหวัด" placeholder="เลือกจังหวัด" class="form-control elder_addr_pre" id="Province" name="center_info[addr_province]" onchange="optionGen(this,'Amphur',<?php echo @$center_info['addr_district']; ?>);"></select>
              <script>
                const PROVINCES = <?=json_encode($this->db->select('area_code,area_name_th,four_regions')->get_where('std_area', array('area_type' => 'Province'))->result_array())?>;
                let filter_provinces_by_region = r => {
                  let filteredProvinces = r ? PROVINCES.filter(p => p.four_regions == r) : PROVINCES;
                  $('#Province').empty().append(
                    '<option value="">เลือกจังหวัด</option>',
                    filteredProvinces.map(p => '<option value="' +p.area_code+'">'+p.area_name_th+'</option>')
                  );
                };
                filter_provinces_by_region();
                $('#FourRegions').change(e => filter_provinces_by_region($(e.target).val()));
              </script>
