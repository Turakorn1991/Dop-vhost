...
class Adaptenvir extends MX_Controller {
	...
	public function adaptenvir_list_ajax($process_action='View') { // รายการสงเคราะห์ผู้สูงอายุในภาวะยากลำบาก
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
		    ...
		    $date_of_finish = '-';
		    if($adaptenvir->date_of_finish!='' && $adaptenvir->date_of_finish!='0000-00-00') {
		      $date_of_finish = '<font class="text-sucsess" color="#18bd15">'.dateChange($adaptenvir->date_of_finish,5).'</font>';
				...
				$date_of_finish = '<font class="text-sucsess" color=red>ปฎิเสธ</font>';
			}
			$row[] = "<center>".$date_of_finish."<center>";
			$req_channel = $this->adaptenvir_model->getOnce_reqChanel($adaptenvir->chn_code);
				

			$row[] = $req_channel['chn_name'];


			if($adaptenvir->insert_org_id =='174')
			{
				if($adaptenvir->update_org_id != null && $adaptenvir->update_org_id != ''){
					$orgU = $this->adaptenvir_model->getOrg($adaptenvir->update_org_id);
					...
					$org2 = $this->adaptenvir_model->getOrg($adaptenvir->insert_org_id);
			...
			{
				$org = $this->adaptenvir_model->getOrg($adaptenvir->insert_org_id);
			...
			$total_budget= $adaptenvir->case_budget + $adaptenvir->mgt_budget;
		    $row[] ="<div style='width:100%;text-align:right;'>".number_format($total_budget,2)."</div>";
      ...
      /*-- End datepicker custom--*/
  		...
  		set_js_asset_footer('webservice.js','personals'); //Set JS webservice.js
			...
			if($process_action=='Add' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				$frm->set_rules('impv_pid[pid]','เลขประจำตัวประชาชน','required');
			...
			// pers_addr
				...
				//	print_r($_POST);exit();
					...
					$data_insert['date_of_svy']    = dateChange($data_insert['date_of_svy']);
					...
					$this->common_model->update('impv_home_info',$data_home_info,array('imp_home_id'=>$id));
					...
					$data_update_pers['reg_addr_id'] = $new_data_id;
// print_r($data_insert_addr);exit();
					...
					$this->common_model->update('pers_info',$data_update_pers,array('pers_id'=>$info_id)); //get_inpost('impv_home_info[pers_id]')
					///////////////////////////////////////////////////
					...
					if(!empty($tmp_family)){
						...
						$this->common_model->delete_where('pers_family','pers_id',get_inpost('impv_home_info[pers_id]'));
						foreach ($tmp_family as $key => $rFml) {
							// dieFont($rFml);

							$fml_insert = array(
							'pers_id'       => $info_id, //get_inpost('impv_home_info[pers_id]'),
					...
					redirect('adaptenvir/agree2/Edit/'.$id,'refresh');
				...
				$req_pers_info = $this->common_model->get_where_custom('pers_info','pers_id',$row['pers_id']);
				...
				$data['addr_reg_info'] = $req_addr_diff_info[0]; //reg_addr_id

				$data['req_prename'] = $this->common_model->getPrename($data['req_pers_info']['pren_code']);
						
				// $pre_addr_diff_info = $this->common_model->get_where_custom('pers_addr','addr_id',$data['req_pers_info']['pre_addr_id']);
				// $data['addr_diff_info'] = $pre_addr_diff_info[0]; //pre_addr_id
			...
			//dieArray($data['addr_diff_info']);	
				...
				if(isset($row['imp_home_id'])) {
					...
					$data['addr_info'] = $this->personal_model->getOnce_PersonalAddress($pers_info['reg_addr_id']);//call register addr
					$data['pre_addr_info'] = $this->personal_model->getOnce_PersonalAddress($pers_info['pre_addr_id']);//call present addr
					$data['pers_info'] = $pers_info;

					$data['impv_home_info']['name'] = $pers_info['name'];
		  $data['impv_home_info']['date_of_birth_num'] = $pers_info['date_of_birth'];			
          if($pers_info['date_of_birth']!='' && checkdate(iconv_substr($pers_info['date_of_birth'],5,2,'utf-8'),iconv_substr($pers_info['date_of_birth'],8,2,'utf-8'),iconv_substr($pers_info['date_of_birth'],0,4,'utf-8'))) {
			...
			$age = $interval->y;
			$data['impv_home_info']['age'] = $age;
			$data['impv_home_info']['date_of_birth'] = "วัน - เดือน - พ.ศ. ".$birthdate." (อายุ ".$age." ปี)";
		}
		  else {
          	$data['impv_home_info']['date_of_birth'] = ' - ';
				...
				if($frm->run($this)){//Valid Data
					...
					$data_update['date_of_svy'] 		= dateChange($data_update['date_of_svy']);

					$position	= $this->input->post("position");
					$org	= $this->input->post("org");
					if($data_update['chn_code'] == '007' && $position == '4'  && $org=='174' ){	
					...
					$info_insert['pid']	= get_inpost('impv_pid[pid]');		
					unset($info_insert['pren_name']);	
					// print_r(get_inpost('impv_home_info[pers_id]'));exit();	//20389110	
				//	$info_id = $this->common_model->insert('pers_info',$info_insert);
					$this->common_model->update('pers_info',$info_insert,array('pers_id'=>get_inpost('impv_home_info[pers_id]')));
					
					//$data_home_info['pid']		= get_inpost('impv_home_info[pers_id]');
					// $data_home_info['pers_id']		= $info_id;
					// print_r($imp_home_id);exit(); //17
					// $this->common_model->update('impv_home_info',$data_home_info,array('imp_home_id'=>$id));
				...
				//	impv_home_info
					...
					//print_r(get_inpost('pers_info[reg_addr_id]'));exit(); //25971825
					$this->common_model->update('pers_addr',$data_addr,array('addr_id'=>get_inpost('pers_info[reg_addr_id]')));
					
					//$new_data_id = $this->common_model->update('pers_addr',$data_addr);
					
					////////////////
					unset($data_update['pid']);
					unset($data_update['pers_id']);
					...
					$data_update_pers = get_inpost_arr('pers_info');
					//dieArray(get_inpost('pre_addr_id'));
					
					//dieArray(get_inpost('pre_addr_id'));
					if(get_inpost('elder_addr_chk')!='on') { //25971826
						//echo "s";
						//add new addr
						...
						$data_update_addr['update_org_id'] 		= get_session('org_id');
						$data_update_addr['update_datetime'] 	= getDatetime();
						//$new_addr_id = $this->common_model->insert('pers_addr',$data_update_addr);
					...
					// if(get_inpost('elder_addr_chk')!='on') {
					// 	//add new
					// 	$impvHome = $this->common_model->get_where_custom('impv_home_info','imp_home_id',$imp_home_id);
					// 	$persInfo = $this->common_model->get_where_custom('pers_info','pers_id',$impvHome[0]['pers_id']);

					// 	if($persInfo[0]['reg_addr_id'] == $persInfo[0]['pre_addr_id']){
					// 		$data_update_addr['addr_home_no']	= get_inpost('pre_addr_id'); 
					// 		$data_update_addr['addr_moo']		= get_inpost('addr_moo_id');  /// $houseNo; 
					// 		$data_update_addr['addr_alley']		= get_inpost('pers_addr[addr_alley]'); 
					// 		$data_update_addr['addr_lane']		= get_inpost('pers_addr[addr_lane]');
					// 		$data_update_addr['addr_road']		= get_inpost('pers_addr[addr_road]');
          ...
          if(!empty($tmp_family)){
            if(current($tmp_family) != ''){
              ...
              foreach ($tmp_family as $key => $rFml) {
                $fml_insert = array(
                  'pers_id'       => get_inpost('impv_home_info[pers_id]'),
                  'fml_pid'   => str_replace("-","",get_inpost("pers_family[fml_pid][{$key}]")),
                  'pren_code' => get_inpost("pers_family[pren_code][{$key}]"),
                  'pers_firstname_th' => get_inpost("pers_family[pers_firstname_th][{$key}]"),
                  'pers_lastname_th' => get_inpost("pers_family[pers_lastname_th][{$key}]"),
			...
			if($process_action=='Add' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				if($frm->run($this)){//Valid Data
					...
					if(get_inpost('elder_addr_chk')!='on') {
						//add new addr
						$data_insert_addr = get_inpost_arr('pers_addr');
						$data_insert_addr['update_user_id']		= getUser();
						$data_insert_addr['update_datetime']	= getDatetime();
		...
		/*-- End datepicker custom--*/
  		...
  		set_js_asset_footer('webservice.js','personals');
			...
			if($process_action=='Edit' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				$req_pers_info = $this->common_model->get_where_custom('pers_info','pers_id',$data['impv_personal_info']['pers_id']);
				$data['req_pers_info'] = $req_pers_info[0];				
				$data['req_prename'] = $this->common_model->getPrename($data['req_pers_info']['pren_code']);
								
				$req_pers_info2 = $this->common_model->get_where_custom('pers_info','pers_id',$data['impv_personal_info']['cns_pers_id']);
				$data['req_agree2'] = $req_pers_info2[0];				
				$data['prename2'] = $this->common_model->getPrename($data['req_agree2']['pren_code']);
				...
				if(isset($row['imp_home_id'])) {
					...
					if($row['cns_pers_id'] == ''){
						...
						$tmp2 = $this->personal_model->getOnce_PersonalAddress($temp['reg_addr_id']);
						...
						//$data['impv_home_info']['reg_add_info'] = @"{$data['addr_info']['addr_home_no']} หมู่ {$data['addr_info']['addr_moo']} ต. {$data['addr_info']['addr_sub_district']} อ. {$data['addr_info']['addr_district']} จ. {$data['addr_info']['addr_province']} {$data['addr_info']['addr_zipcode']}";
						$data['impv_home_info']['reg_add_info'] = $tmp2 ['addr_home_no'];
						$data['impv_home_info']['reg_add_info'] .= ($tmp2 ['addr_moo'] != '')?' หมู่'.$tmp2['addr_moo']:'';
						$data['impv_home_info']['reg_add_info'] .= ($tmp2['addr_sub_district'] != '')?' ต.'.$tmp2['addr_sub_district']:'';
						$data['impv_home_info']['reg_add_info'] .= ($tmp2['addr_district'] != '')?' อ.'.$tmp2['addr_district']:'';
						$data['impv_home_info']['reg_add_info'] .= ($tmp2['addr_province'] != '')?' จ.'.$tmp2['addr_province'].' '.$tmp2['addr_zipcode']:'';
						$data['impv_home_info']['reg_add_info'] = ($data['impv_home_info']['reg_add_info']!='')?$data['impv_home_info']['reg_add_info']:'-';
						$data['reg_addr'] = $this->common_model->get_where_custom('pers_addr','addr_id',$data['pers_info']['reg_addr_id'] );
				...
				if($frm->run($this)){//Valid Data
					...
					$data_update = get_inpost_arr('impv_home_info');
					$data_update['date_of_consi'] 	= dateChange($data_update['date_of_consi']);
					$data_update['update_user_id'] 	= getUser();
					$data_update['update_org_id'] 	= get_session('org_id');
					$data_update['update_datetime'] = getDatetime();					
					//$data_update['cns_pers_id'] 	= "";				
					//dieArray($data_update);
					$this->common_model->update('impv_home_info',$data_update,array('imp_home_id'=>$imp_home_id));

					$info_insert = array();
					$info_insert = get_inpost_arr("impv_info");
					$pid = str_replace("-","",get_inpost('impv_info[pid]'));	
					$info_insert['pid']	= $pid;
					$list_info = $this->common_model->get_where_custom('impv_home_info','imp_home_id',$imp_home_id);
					// $info_id ="";
					unset($info_insert['pren_name']);
					if($list_info[0]['cns_pers_id'] != 0){						
					 	 $this->common_model->update('pers_info',$info_insert,array('pers_id'=>$list_info[0]['cns_pers_id']));
						 $data_home_info['cns_pers_id']		= $list_info[0]['cns_pers_id'];
					}else{
						$info_id = $this->common_model->insert('pers_info',$info_insert);
						$data_home_info['cns_pers_id']		= $info_id;
					}		
						
					//$data_home_info['pers_id'] 	= $info_id;
					// //$data_home_info['pid']		= get_inpost('impv_home_info[pers_id]');
					
					$this->common_model->update('impv_home_info',$data_home_info,array('imp_home_id'=>$imp_home_id));
					//pers_addr
					//// UPDATE ADDRESS AND PERS_INFO //////////////////////////////
					$data_update_pers = get_inpost_arr('pers_info');

					$list_addr2 = $this->common_model->get_where_custom('pers_info','pers_id',$data_home_info['cns_pers_id']);
					
					//$list_addr2['0']['reg_addr_id']
					//$list_addr2['0']['pre_addr_id']
					//pre_addr_id
					///////////////////////reg_addr///////////////////////////////
					$data_addr = get_inpost_arr('impv_addr');
					$data_addr['insert_user_id']		= getUser();
					...
					if($list_addr2['0']['reg_addr_id'] != ""){
						// echo "d";
						unset($data_update_pers['reg_addr_id']);
						$this->common_model->update('pers_addr',$data_addr,array('addr_id'=>$list_addr2['0']['reg_addr_id']));
					...
					//	echo "ssd";
						...
						if($list_addr2['0']['pre_addr_id'] == ""){
							$new_addr_id = $this->common_model->insert('pers_addr',$data_insert_addr);
							$data_update_pers['pre_addr_id'] = $new_addr_id;
						}else{
							// dieArray($reg_addr_id);
							unset($data_update_pers['pre_addr_id']);
							$this->common_model->update('pers_addr',$data_insert_addr,array('addr_id'=>$list_addr2['0']['pre_addr_id']));
				...
				//	$data_update_pers['reg_addr_id'] = $new_data_id;
					...
					// 		// $house_no =   get_inpost('pers_addr[addr_moo]'); 
					// 		// $houseNo = str_replace("หมู่ที่ ", "", $house_no);
					// 		$data_insert_addr['addr_home_no']		= get_inpost('impv_addr[addr_home_no]'); 
					// 		$data_insert_addr['addr_moo']		= get_inpost('impv_addr[addr_moo]');  /// $houseNo; 
					// 		$data_insert_addr['addr_alley']		= get_inpost('impv_addr[addr_alley]'); 
					// 		$data_insert_addr['addr_lane']		= get_inpost('impv_addr[addr_lane]');
					// 		$data_insert_addr['addr_road']		= get_inpost('impv_addr[addr_road]');
					...
					// 		//print_r($data_insert_addr);exit();
					// 		//pers_info['pre_addr_id'];
					// 		$agree2_addr_id = $this->common_model->insert('pers_addr',$data_insert_addr);
							
					// 		$data_update_pers['pre_addr_id'] = $agree2_addr_id;
					// 		$data_update_pers['reg_addr_id'] = $agree2_addr_id;
					...
					// unset($data_update_pers['email_addr']);
					//unset($data_update_pers['tel_no_mobile']);
					$this->common_model->update('pers_info',$data_update_pers,array('pers_id'=>$data_home_info['cns_pers_id']  ));
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
			...
			if($process_action=='Edit' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				if($frm->run($this)){//Valid Data
					...
					$data_update = get_inpost_arr('impv_home_info');
					//$data_update['date_of_finish'] 	= dateChange($data_update['date_of_finish']);
					if($data_update['consi_result'] == "อนุมัติ"){
						$data_update['date_of_finish'] = dateChange($data_update['date_of_finish']);
					}else{
						$data_update['date_of_finish'] = null ;
					}
					$data_update['update_user_id'] 	= getUser();
					$data_update['update_org_id'] 	= get_session('org_id');
					$data_update['update_datetime'] = getDatetime();
					...
					if($_FILES['img']['name'][0]!=""){
						...
						if ($_FILES['img']['name'][0]!="")
						{ 
							$photo2info = $this->files_model->getMultiImg("img",'assets/modules/adaptenvir/images/uploads');
							...
							foreach ($photo2info as $key_photo => $value_photo) {
									...
									if($_FILES['img']['name'][$i]!=""){
										...
										$insert_photo = array('impv_home_id' 		    => $id,
																'impv_home_info_title'	=> $update_photo[$j],
	...
	public function delete_impv_photo(){
	
		$id_photo = $this->input->get('photo_id');//filename

		$photo_file_name = $this->common_model->custom_query("select impv_home_photo_file from impv_home_info_photo where impv_home_photo_id = {$id_photo}");

		$impv_home_id = $this->common_model->custom_query("select impv_home_id from impv_home_info_photo where impv_home_photo_id = {$id_photo}");
		 
		$impv_id = $impv_home_id[0]['impv_home_id'];

		$photo_file = $photo_file_name[0]['impv_home_photo_file'];
		
		$this->load->helper('string');

 		$rpath = $_SERVER['DOCUMENT_ROOT'];

		$path = increment_string($rpath, '/assets/modules/adaptenvir/images/uploads/', $photo_file);
		...
		redirect('adaptenvir/assist3/Edit/'.$impv_id,'refresh');
...
/* 	public function del_impv_photo(){
	...
	public function getarea() { 
		...
		$province_code = $this->adaptenvir_model->getAreaID('Province',$input['province_name']);
		$rs['province_code']  = $province_code['area_code'];

		$amphur_code = $this->adaptenvir_model->getAreaID('Amphur',$input['amphur_name']);		
		$rs['amphur_code']  = $amphur_code['area_code'];

		$tambol_code = $this->adaptenvir_model->getAreaID('Tambon',$input['tambol_name']);		
		$rs['tambol_code']  = $tambol_code['area_code'];
	...
	public function getBirthday() { 
		$input = $this->input->post('birthday');		
		$data = explode("-", $input);
		$month = formatDateThaiMonthOnly($data[1]);
		$nowYear = date("Y")+543;
		$old = $data[2] - $nowYear;
		$date = $data[2].' '.$month.' '.$data[0]; //.'(อายุ '.$old.' ปี)'
	...
	public function getToken() { 
		$user_id = get_session('user_id');
		$usrm_user = $this->common_model->get_user($user_id);	
		
		$consumerSecret = $usrm_user['consumer_secret'];
		$agentID = $usrm_user['agent_id'];

		$head = array(
			'Content-Type' => 'application/x-www-form-urlencoded', //'application/x-www-form-urlencoded', 
			'Consumer-Key' => '9f3d707a-e980-4691-816e-b5f860a1f99a',
		);
		//$response = Requests::get('https://api.egov.go.th/ws/auth/validate?ConsumerSecret='.$consumerSecret.'&AgentID='.$agentID.'',$head); 
		$response = Requests::get('https://api.egov.go.th/ws/auth/validate?ConsumerSecret=ig7T9zeLZXb&AgentID='.$agentID.'',$head); 
	...
	public function getDataPid() { 
		...
		$token="18a6c18a-9581-431f-9be4-ecb470ded384";
		$headers  = [           
		...
		$pid = str_replace("-","",$input['req_pid']);	
		$response = Requests::get('https://api.egov.go.th/ws/dopa/linkage/v1/link?OfficeID=00023&ServiceID=001&Version=01&CitizenID='.$pid,$headers); 
		$result = json_decode($response->body);
		$date = $result;
	...
	public function getAddress() { 
		...
		$headers  = [           
			'Content-Type' =>  'application/x-www-form-urlencoded',
			'Consumer-Key' =>  '9f3d707a-e980-4691-816e-b5f860a1f99a',
			'Token' => $input['tokenId'] //'0d947376-8bf3-4430-8ad2-4f43119dfba6'
		];
		
		$pid = str_replace("-","",$input['req_pid']);	
		$response = Requests::get('https://api.egov.go.th/ws/dopa/linkage/v1/link?OfficeID=00023&ServiceID=027&Version=01&CitizenID='.$pid,$headers); 
		$result = json_decode($response->body);
		$date = $result;
	...
	public function getTitleName() { 	
		...
		$input = $this->input->post('title_name');	
		$title_code = $this->common_model->getTitleID($input);
		$data = $title_code;
	...
	public function getDataDouble() { 
		$input = $this->input->post();		
		//$rs = $this->common_model->getAdaptenvirDouble($input['req_id']);
		$rs = null;
		$this->output
            ->set_content_type('application/json')
            ->set_output(json_encode($rs));
	...
	private static function checkDate($dateStr){
        if($dateStr == '' || $dateStr == null){
            return "";
        }
        else{
            $dateAr = explode(" ", $dateStr);
			if(count($dateAr) == 3){
				$dateM = "";
				switch ($dateAr[1]) {
					case "มกราคม":
				...
				$dateY = (string)((int)$dateAr[2] - 543);
				return $dateY."-".$dateM."-".(string)$dateAr[0];
			}else{
				return $dateStr;
			}
        }
