...
class Difficult extends MX_Controller {
	...
	public function assist_list_ajax($process_action='View') { // รายการสงเคราะห์ผู้สูงอายุในภาวะยากลำบาก
      ...
      // echo count($list);
			...
			foreach ($list as $i=>$manage_transfer) {
				...
				$row = array();
				$diff_id = array();
				$diff_id = $manage_transfer->diff_id;

			    $row[] = "<center>".$no."</center>";
                $row[] = $manage_transfer->pid;
				$row[] = $manage_transfer->prename_th." ".$manage_transfer->name;
				$row[] = $manage_transfer->gender_code;
				
				$age = '';
				
				if($manage_transfer->date_of_birth !='' && $manage_transfer->date_of_birth != null) {
				...
				else if($manage_transfer->payment_status =='ปฎิเสธ'){
				...
				$req_channel = $this->difficult_model->getOnce_reqChanel2($manage_transfer->chn_code);
				...
				$diff_id = $manage_transfer->diff_id;
				$diff = $this->difficult_model->get_diffInfo($diff_id);
				if($diff['insert_org_id'] =='174')
				{
					if($diff['update_org_id'] != null && $diff['update_org_id'] != '')
					{
						$orgU = $this->difficult_model->getOrg($diff['update_org_id']);
					...
					{
						$org2 = $this->difficult_model->getOrg($diff['insert_org_id']);
					...
					$org = $this->difficult_model->getOrg($diff['insert_org_id']);
			...
			//set_session('filter_diff_id',$output2);
	...
	// V1
	public function send_sms_old($msisdn,$message,$id,$page){
	...
	public function send_sms($msisdn,$message,$id,$page){
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_POSTFIELDS => array('msisdn' => $msisdn,'message' => $message,'sender' => 'GoldbyDOP','force ' => 'corporate'),
			CURLOPT_HTTPHEADER => array(
			  'Authorization: Basic ZWUwNDBkOTFlYTM3MzJjYWJkMGE3ODQ4ZjBkNjgyNjk6ZDZjMWM2OWU5ZWU5ZTdiZTEzNDAwZDgxMGZiYzFlZTc='
			),
		  ));
		  ...
		  if($code['http_code'] == 201){
			...
			$msg_string = "สามารถส่งออกได้จำนวน ".count($data->phone_number_list)." หมายเลข, ใช้เครดิตทั้งหมด ".$data->total_use_credit." เครดิต";
		  }
		  else{
			$msg_string = "เกิดข้อผิดพลาดในการทำงาน (".$data->error->description.")";
		  }
		  return $msg_string;
	}
  ...
  public function getElderyJPTH($process_action='View') {
		...
		/*-- End datepicker custom--*/
  		...
  		set_js_asset_footer('mapmarker.js','webconfig'); //Set JS mapmarker.js --
			...
			if($process_action=='Add' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				$frm->set_rules('info_pid[pid]','เลขประจำตัวประชาชน (ผู้แจ้งเรื่อง)','required');
				...
				if($frm->run($this)){//Valid Data
					...
					$data_insert['pid']	= get_inpost('info_pid[pid]');
					//req_tel_no
					...
					if(get_inpost('elder_pers_chk')!='on') {						
						...
						$data_addr_pers2['reg_addr_id'] =$new_addr_id; //$update_pers['pre_addr_id'];
				...
				//	dieArray($data_home_info); //20388598
					//$data_home_info['pid']		= get_inpost('diff_info[pers_id]');
					$data_home_info['req_pers_id']		= $info_id;
					$this->common_model->update('diff_info',$data_home_info,array('diff_id'=>$id));
					//exit();
					
					$this->common_model->update("pers_info",$data_addr_pers2,array('pers_id'=>$info_id2));
					//dieArray($data_update_pers2);
					...
					//$this->common_model->update('pers_info',$update_pers,array('pers_id'=>$info_id2)); //get_inpost('impv_home_info[pers_id]')
	...
	// dieArray($data_insert); 
					...
					if(!empty($tmp_family)){
						if(current($tmp_family) != ''){
							foreach ($tmp_family as $key => $rFml) {
								// dieFont($rFml);
								$fml_insert = array(
									'pers_id' 			=> $info_id2, //get_inpost('diff_info[pers_id]'),
					...
					if($sms_status == "true"){
						...
						$no = get_inpost('pers_info[tel_no]');
						$page=1;
				...
				if(isset($row['diff_id'])) {
					$data['diff_info'] = $row;
					//$addr_diff_info = $this->common_model->get_where_custom('pers_addr','addr_id',$diff_id);
					$addr_diff_info = $this->common_model->get_where_custom('pers_addr','addr_id',$data['diff_info']['reg_addr_id']);
					$data['addr_diff_info'] = $addr_diff_info[0];

					 $req_pers_info = $this->common_model->get_where_custom('pers_info','pers_id',$data['diff_info']['req_pers_id']);
					 $data['req_pers_info'] = $req_pers_info[0];
					 
					 $req_pers_detail = $this->common_model->get_where_custom('pers_info','pers_id',$data['diff_info']['pers_id']);
					 $data['diff_detail'] = $req_pers_detail[0];
					// dieArray( $data['diff_detail']  );	
					...
					if($row['date_of_birth']!='' && checkdate(iconv_substr($row['date_of_birth'],5,2,'utf-8'),iconv_substr($row['date_of_birth'],8,2,'utf-8'),iconv_substr($row['date_of_birth'],0,4,'utf-8'))) {
						...
						$interval = $now->diff($date);
						$age = $interval->y;
						$data['diff_info']['age'] = $age;
						$data['diff_info']['date_of_birth'] = formatDateThai($row['date_of_birth']).' (อายุ '.$age.' ปี)';
						if($age >= 60){
							$data['diff_info']['age_status']="60 ปีขึ้นไป";
						}
						else{
							$data['diff_info']['age_status']="น้อยกว่า60ปี";
						}
					}
						...
						$age = $interval->y;
						$data['diff_info']['age'] = $age;
						$data['diff_info']['date_of_birth'] = "วัน - เดือน - พ.ศ. ".$birthdate." (อายุ ".$age." ปี)";
						if($age >= 60){
							$data['diff_info']['age_status']="60 ปีขึ้นไป";
						}
						else{
						...
						$data['diff_info']['age_status']=' - ';
					}
					if($row['date_of_death']==''){
						$data['diff_info']['death_date']= " : ".formatDateThai($row['date_of_death']);
						$data['diff_info']['death_status']='<span style="color:green">มีชีวิตอยู่</span>';
					}
					else{
						$data['diff_info']['death_date']= ' - ';
						$data['diff_info']['death_status']='<span style="color:red">เสียชีวิต</span>';
					}
					...
					$data['diff_info']['tel_no_mobile'] = $row['tel_no'];
					$data['diff_info']['email_address'] = $row['email_addr'];
					...
					if(isset($tmp['pid'])) {
						...
						$data['diff_info']['date_of_birth_num'] = $row['date_of_birth'];
						...
						$data['diff_info']['req_date_of_birth'] = formatDateThai($tmp['date_of_birth']).' (อายุ '.$age.' ปี)';
							...
							$interval = $now->diff($date);
							$age = $interval->y;
							$data['diff_info']['req_date_of_birth'] = "วัน - เดือน - พ.ศ. ".$birthdate." (อายุ ".$age." ปี)";
						}
						else {
							$data['diff_info']['req_date_of_birth'] = ' - ';
						}
            			...
            			$data['diff_info']['req_tel_no'] = $tmp['tel_no'];
				...
				if($frm->run($this)){//Valid Data
					...
					//if($data_update['chn_code'] == '007' && $this->input->post("position") == '4' && $data_update['chn_code'] == '007' && $this->input->post("position") == '2' ){
					$position	= $this->input->post("position");
					$org	= $this->input->post("org");
					$row = $this->difficult_model->get_diffInfo($diff_id);
					...
					$update_req_pers['update_org_id'] 	= get_session('org_id');
					$update_req_pers['update_datetime'] = getDatetime();					
					$this->common_model->update("pers_info",$update_req_pers,array('pers_id'=>$row['req_pers_id']));
					
					/////////////////////name1/////////////////
					$info_insert = get_inpost_arr("impv_info");
					unset($info_insert['req_reg_addr_id']);
					unset($info_insert['pren_name']);
					//print_r($row['req_pers_id']);exit();  //86
					$info_insert['pid']	= get_inpost('info_pid[pid]');							
					...
					//$req_pers_info2 = $this->common_model->get_where_custom('pers_info','pers_id',$data_update['req_pers_id']);
					//$req_addr_info = $req_pers_info[0];		
					//print_r($row['pers_id']);exit();					
					...
					if(get_inpost('elder_addr_chk')!='on') {
						...
						$new_addr_id = $this->common_model->update('pers_addr',$data_update_addr,array('addr_id'=>get_inpost('pre_addr_id')));
						// $new_addr_id = $this->common_model->insert('pers_addr',$data_insert_addr);
						...
						//$data_update_addr['pre_addr_id'] = $data_update_pers['reg_addr_id'];
					}					
					...
					if($sms_status == "true"){
						...
						$no = get_inpost('pers_info[tel_no]');
						$page = 1;
						$this->send_sms($no,$msg,$diff_id,$page);
					}
  		...
  		/*-- End Toastr style --*/
			...
			if($process_action=='Edit' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				if(isset($row['diff_id'])) {
					...
					$org_info = $this->difficult_model->getOrg($row['visitor_org_id']);
					$data['diff_info']['visitor_org_id'] = $org_info['org_title'];
					$user_info = $this->difficult_model->getUsrm($row['visitor_user_id']);
					$data['diff_info']['visitor_user_id'] = $user_info['user_firstname'].' '.$user_info['user_lastname'];
				...
				// if(0){//Valid Data
					...
					$data_update['update_org_id'] 	= get_session('org_id');
					$data_update['update_datetime'] = getDatetime();
					$data_update['visitor_user_id'] 	= getUser();
					$data_update['visitor_org_id'] 	= get_session('org_id');
					$this->common_model->update('diff_info',$data_update,array('diff_id'=>$diff_id));
	...
	public function sufferer_form3($process_action='Add',$diff_id=0) { //แบบขอรับบริการ (สงเคราะห์)
      ...
      /*-- End datepicker --*/
			...
			if($process_action=='Edit' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				if(isset($row['diff_id'])) {
					$data['diff_info'] = $row;
					$org_info= $this->difficult_model->getOrg($row['insert_org_id']);
					$data['diff_info']['org_title'] = $org_info['org_title'];
				...
				//if($frm->run($this)){//Valid Data
				 //if(0){//Valid Data
					...
					if($data_update['payment_status'] == "อนุมัติ"){
					...
					$sms_status  = $this->input->post("sms");
					 if($sms_status == "true"){
						 $page=2;
						 $row = $this->difficult_model->getOnce_diffInfo($diff_id);
						 $person = $this->personal_model->getOnce_PersonalInfo($row['pers_id']);
					 	 $no = $person['tel_no'];
						if(get_inpost('diff_info[payment_status]')=='อนุมัติ'){
							$msg = "คำขอได้รับการอนุมัติ หากโอนเงินแล้วระบบจะส่งข้อความแจ้งเตือนอีกครั้ง";
						}
						else{
							$msg = "คำขอได้รับการปฏิเสธ กรุณาติดต่อหน่วยงานที่รับเรื่อง";
						}
						$this->send_sms($no,$msg,$diff_id,$page);
					}
					$this->session->set_flashdata('msg',setMsg('021')); //Set Message code 021 อัปเดตข้อมูลเสร็จสิ้น
	...
	public function getDataDead() { 
		...
		//$rs = $this->common_model->getDifficultDouble($input['req_id']);
		$rs = null;
		$this->output
