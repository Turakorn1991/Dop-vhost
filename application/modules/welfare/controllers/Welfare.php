...
class Welfare extends MX_Controller {
	...
	public function getElderyJPTH($process_action='View') {
		...
		$app_id = 3;
		$process_path = 'welfare/getElderyJPTH';
  ...
  public function getReqHistory($process_action='View') {
		...
		$row = array();
		  ...
		  if($tmp_irp['date_of_irp']!='' && $tmp_irp['date_of_irp'] != '0000-00-00'){
			...
			$row[] = "<center>".$date_irp."</center>";
				  ...
				  $req_channel = $this->welfare_model->getOnce_reqChanel($welfare->chn_code);
		
				$row[] = $req_channel['chn_name'];
				
				   $adm = $this->welfare_model->get_AdmInfo($welfare->adm_id);
				$org = $adm['insert_org_id'];
				$orgUpdate = $adm['update_org_id'];
				if($org =='174'){
					if($orgUpdate != null && $orgUpdate != ''){
						$orgU = $this->welfare_model->getOrg($orgUpdate);
						if($orgU['org_short_title'] != ''){
							$row[]= $orgU['org_short_title'];
						}else{
							$row[]= $orgU['org_title'];
						}
					}else{
						$org2 = $this->welfare_model->getOrg($org);
						if($org2['org_short_title'] != ''){
							$row[]  = '<font class="text-sucsess" color="#B9B9B9">'.$org2['org_short_title'].'</font>';
						}else{
							$row[]  = '<font class="text-sucsess" color="#B9B9B9">'.$org2['org_title'].'</font>';
						}
					}
				}else{
					$adm = $this->welfare_model->getOrg($org);
					if($adm['org_short_title'] != ''){
						$row[]= $adm['org_short_title'];
					}else{
						$row[]= $adm['org_title'];
					}							
				}
	...
	private function clr_persPhysicality_form1(){
		return array(
			'pers_id'									=>	'',
			'phy_face_detail'					=>	'',
			'phy_skin_detail'					=>	'',
			'phy_mark_detail'					=>	'',
			'phy_interaction_detail'	=>	'',
			'phy_expression_detail'		=>	''
	...
	private function clr_persHealth_form1(){
		 return array(
				'pers_id'								=>	'',
				'pers_height'						=>	'',
				'pers_weight'						=>	'',
				'pers_physical_condition'=>	'',
				'pers_mental_condition'	=>	'',
				'pers_congenital_disease'=>	''
	...
	private function clr_admInfo_form1() {
		...
		if(@$usrpm['perm_status']=='No' || !isset($usrpm['app_id'])){
			...
			$this->webinfo_model->LogSave($app_id,$process_action,'Sign Out','Fail'); //Save Sign Out Log
					...
					/*-- End datepicker custom--*/
						...
						set_css_asset_head('../modules/welfare/css/gallery_img.css');
					...
					if($process_action=='Add' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
						...
						$data['pers_physicality']		= $this->clr_persPhysicality_form1();
						$data['pers_health_record']	= $this->clr_persHealth_form1();
				...
				if($frm->run($this)){//Valid Data
					...
					unset($data_insert['req_tel_no']);
					if($_FILES['img']['name']!=''){
						$namephoto = $this->files_model->getOnceImg("img",'assets/modules/welfare/uploads');
						 
						 $data_insert['adm_photo_file'] = 'assets/modules/welfare/uploads/'.$namephoto;
						 $data_insert['adm_photo_label'] = $_FILES['img']['name'];
						 $data_insert['adm_photo_size'] = $_FILES['img']['size'];
					}
					if($_FILES['img2']['name']!=''){
						 $namephoto2 = $this->files_model->getOnceImg("img2",'assets/modules/welfare/uploads');
					
						 $data_insert['adm_req_photo_file'] = 'assets/modules/welfare/uploads/'.$namephoto2;
						 $data_insert['adm_req_photo_label'] = $_FILES['img2']['name'];
						 $data_insert['adm_req_photo_size'] = $_FILES['img2']['size'];
					...
					$info_insert['pid']	= get_inpost('info_pid[pid]');	
					unset($info_insert['pren_name']);				
					unset($info_insert['req_reg_addr_id']);
					
					$info_id = $this->common_model->insert('pers_info',$info_insert); //ชื่อบน
					//$this->common_model->delete_where('pers_physicality','pers_id',get_inpost('adm_info[pers_id]'));
					$data_phy_insert							= array();
					$data_phy_insert 							= @get_inpost_arr('pers_physicality');
					$data_phy_insert['pers_id']					= $info_id;//get_inpost('diff_info[pers_id]');
					
					$this->common_model->insert('pers_physicality',$data_phy_insert);
					
					//insert pers health record
					//$this->common_model->delete_where('pers_health_record','pers_id',get_inpost('adm_info[pers_id]'));
					$data_health_insert							= array();
					$data_health_insert							= @get_inpost_arr('pers_health_record');
					$data_health_insert['pers_id']				= $info_id; //get_inpost('adm_info[pers_id]');
					//dieArray($data_health_insert);
					$this->common_model->insert('pers_health_record',$data_health_insert);
					//insert pers disability
					$tmp_dis = get_inpost_arr('pers_disability[dis_code]');
					if(!empty($tmp_dis)){
						$this->common_model->delete_where('pers_disability','pers_id',$info_id);
						foreach ($tmp_dis as $row) {
								$dis_insert = array(
								'pers_id'			=> $info_id,//get_inpost('adm_info[pers_id]'),
								'dis_code'		    => $row,
					...
					$info_insert2['pid']	= get_inpost('diff_pid[pid]');
					$info_insert2['pren_code']	= get_inpost('impv_info2[pren_code_sufferer]');
					unset($info_insert2['pren_code_sufferer']);

					unset($info_insert2['req_reg_addr_id']);

					$info_id2 = $this->common_model->insert('pers_info',$info_insert2); //ชื่อ2
					...
					$update_req_pers['reg_addr_id'] = $new_data_id;
					$data_addr_pers2['pre_addr_id'] = $new_addr_id;
					
					/////////////////////addr2////////////////////////////
					
					if(get_inpost('elder_pers_chk')!='on') {
						...
						// 	$info_id2 = $this->common_model->insert('pers_info',$info_insert2); //ชื่อ2
						// 	$data_home_info['pers_id']		= $info_id2;
						// 	$data_addr_pers2['pre_addr_id'] = $update_req_pers['reg_addr_id'] ;
						// }else{
						// 	$data_home_info['pers_id']		= $data_home_info['pers_id'];
						// 	$data_addr_pers2['pre_addr_id'] = $data_addr_pers2['pre_addr_id'];
						// }
						//dieArray($data_home_info); //20388913
						$data_insert_addr2 = get_inpost_arr('impv_addr2');
						$data_insert_addr2['insert_user_id']		= getUser();
						$data_insert_addr2['insert_datetime']	= getDatetime();
						$data_insert_addr2['insert_org_id'] 		= get_session('org_id');
						
						$new_addr_id = $this->common_model->insert('pers_addr',$data_insert_addr2);															
						$data_addr_pers2['reg_addr_id'] = $new_addr_id;
						//$data_addr_pers2['pre_addr_id'] = $update_req_pers['reg_addr_id'] ;
						//$data_update_pers2['pre_addr_id'] =$update_pers['pre_addr_id'];
					//	dieArray($data_addr_pers2); //20388913
					}else{

						$data_insert_addr2 = get_inpost_arr('impv_addr2');
						$data_insert_addr2['addr_home_no']		= get_inpost('impv_addr2[addr_home_no]'); 
						$data_insert_addr2['addr_moo']		= get_inpost('impv_addr2[addr_moo]');  /// $houseNo; 
						$data_insert_addr2['addr_alley']		= get_inpost('impv_addr2[addr_alley]'); 
						$data_insert_addr2['addr_lane']		= get_inpost('impv_addr2[addr_lane]');
						$data_insert_addr2['addr_road']		= get_inpost('impv_addr2[addr_road]');
						$data_insert_addr2['insert_user_id']		= getUser();
						$data_insert_addr2['insert_datetime']	= getDatetime();
						$data_insert_addr2['insert_org_id'] 		= get_session('org_id');
						...
						// 	$data_addr_pers2['reg_addr_id'] = $update_req_pers['reg_addr_id'];
						// 	$data_update_pers2['pre_addr_id'] = $update_req_pers['reg_addr_id'];						
					...
					if(get_inpost('elder_addr_chk')!='on') {						
						...
						$data_insert_addr['insert_datetime']	= getDatetime(); //addr3
						...
						$data_addr_pers2['pre_addr_id'] = $new_addr_id;
						//if($_POST['pre_addr_id'] != ""){  ///ดึงข้อมูลหน้าบัตร
						// 	$data_insert_addr = get_inpost_arr('impv_addr');
						// 	$data_insert_addr['insert_user_id']		= getUser();
						...
						// 	$update_req_pers['reg_addr_id'] = $new_addr_id;
						
						// }else{
						// 	$update_req_pers['reg_addr_id'] = $update_req_pers['reg_addr_id'];
						// }
					}
					
					$this->common_model->update("pers_info",$update_req_pers,array('pers_id'=>$info_id));


					
				
					$data_home_info['req_pers_id']		= $info_id;
					
					$this->common_model->update('adm_info',$data_home_info,array('adm_id'=>$id));				
					$this->common_model->update("pers_info",$data_addr_pers2,array('pers_id'=>$info_id2));
				//	$this->common_model->update("pers_info",$data_update_pers2,array('pers_id'=>$info_id));
					...
					$data_update_pers['pre_addr_status'] =  $data_update_pers['pre_addr_status'];
					$data_update_pers['tel_no'] =  $data_update_pers['tel_no'];
					//unset($data_update_pers['tel_no']);
					//unset($data_update_pers['email_addr']);
                     
					//print_r($data_update_pers);exit();
					unset($data_update_pers['reg_addr_id']);
					$this->common_model->update('pers_info',$data_update_pers,array('pers_id'=>$info_id2)); //get_inpost('impv_home_info[pers_id]')
          ...
          if(!empty($tmp_family)){
            if(current($tmp_family) != ''){
              ...
              foreach ($tmp_family as $key => $rFml) {
                ...
                $fml_insert = array(
                  'pers_id'       => $info_id,//get_inpost('diff_info[pers_id]'),
				  ...
				  'fml_tel_no_home'	=> get_inpost("pers_family[fml_tel_no_home][{$key}]"),
				  'fml_tel_no'	=> get_inpost("pers_family[fml_tel_no][{$key}]"),
				  
				  'fml_pre_addr_status' => get_inpost("pers_family[fml_pre_addr_status][{$key}]"),
				  'fml_addr_home_no' => get_inpost("pers_family[fml_addr_home_no][{$key}]"),
				  'fml_addr_moo' => get_inpost("pers_family[fml_addr_moo][{$key}]"),
				  'fml_addr_alley' => get_inpost("pers_family[fml_addr_alley][{$key}]"),
				  'fml_addr_lane' => get_inpost("pers_family[fml_addr_lane][{$key}]"),
				  'fml_addr_road' => get_inpost("pers_family[fml_addr_road][{$key}]"),
				  'fml_addr_province' => get_inpost("pers_family[fml_addr_province][{$key}]"),
				  'fml_addr_district' => get_inpost("pers_family[fml_addr_district][{$key}]"),
				  'fml_addr_sub_district' => get_inpost("pers_family[fml_addr_sub_district][{$key}]"),
				  'fml_addr_zipcode' => get_inpost("pers_family[fml_addr_zipcode][{$key}]")
                );
		...
		//   exit();
          /////////////////////////////////////////////////////////////////
					...
					$this->webinfo_model->LogSave($app_id,$process_action,'Sign Out','Success'); //Save Sign Out Log
					redirect('welfare/admission2/Edit/'.$id,'refresh');
				...
				$data['pers_disability'] 	= $this->welfare_model->getAll_Disability($row['req_pers_id']);
				$data['pers_physicality'] = $this->welfare_model->getAll_Physicality($row['req_pers_id']);
				$data['pers_health_record'] = $this->welfare_model->getAll_HealthRecord($row['req_pers_id']);

				//dieArray($data['pers_health_record'] );
				if(isset($row['adm_id'])) {
					
						$data['adm_info'] 			= $row;

						////////////////////////						
						$addr_diff_info = $this->common_model->get_where_custom('pers_addr','addr_id',$data['adm_info']['reg_addr_id']);
						$data['addr_diff_info'] = $addr_diff_info[0];
						
						$req_pers_info = $this->common_model->get_where_custom('pers_info','pers_id',$data['adm_info']['req_pers_id']);
						...
						$req_pers_detail = $this->common_model->get_where_custom('pers_info','pers_id',$data['adm_info']['pers_id']);
						$data['diff_detail'] = $req_pers_detail[0];
					
						
						$data['req_prename'] = $this->common_model->getPrename($data['req_pers_info']['pren_code']);
						$data['diff_prename'] = $this->common_model->getPrename($data['diff_detail']['pren_code']);
					  
						////////////////
						$data['adm_info']['name'] 	= $row['name'];
						$row = $this->welfare_model->getOnce_admInfo($adm_id);
						$pers_information 			= $this->personal_model->getOnce_PersonalInfo($row['pers_id']);
								
						$data['addr_info']= $this->personal_model->getOnce_PersonalAddress($row['pre_addr_id']);
						
						
		            if($row['date_of_birth']!='' && checkdate(iconv_substr($row['date_of_birth'],5,2,'utf-8'),iconv_substr($row['date_of_birth'],8,2,'utf-8'),iconv_substr($row['date_of_birth'],0,4,'utf-8'))) {
		              ...
		              $data['adm_info']['date_of_birth'] = formatDateThai($row['date_of_birth']).' (อายุ '.$age.' ปี)';
					...
					else if($row['date_of_birth']!='' && iconv_substr($row['date_of_birth'],5,2,'utf-8')!= ''){
						$birthdate = iconv_substr($row['date_of_birth'],0,4,'utf-8')+543; 
						$date = new DateTime($row['date_of_birth']);
						$now = new DateTime();
						$interval = $now->diff($date);
						$age = $interval->y;
						$data['adm_info']['date_of_birth'] = "วัน - เดือน - พ.ศ. ".$birthdate." (อายุ ".$age." ปี)";
					}
					else {
		            	$data['adm_info']['date_of_birth'] = ' - ';
									...
									$data['adm_info']['pre_addr_status'] = $row['pre_addr_status'];
									...
									$data['adm_info']['reg_add_info'] = $tmp2['addr_home_no'];
									$data['adm_info']['reg_add_info'] .= ($tmp2['addr_moo']!='')?' หมู่'.$tmp2['addr_moo']:'';
									$data['adm_info']['reg_add_info'] .= ($tmp2['addr_moo']!='')?' ต.'.$tmp2['addr_sub_district']:'';
									$data['adm_info']['reg_add_info'] .= ($tmp2['addr_moo']!='')?' อ.'.$tmp2['addr_district']:'';
									$data['adm_info']['reg_add_info'] .= ($tmp2['addr_moo']!='')?' จ.'.$tmp2['addr_province'].' '.$tmp2['addr_zipcode']:'';
									$data['adm_info']['reg_add_info'] = ($data['adm_info']['reg_add_info']!='')?$data['adm_info']['reg_add_info']:'-';
								...
								//dieArray($data['adm_info']);
									...
									$data['adm_info']['req_tel_no'] 			= $row['tel_no'];
					...
					if(isset($tmp['pid'])) {
						...
						$data['adm_info']['date_of_birth_num'] = $row['date_of_birth'];
			            if($tmp['date_of_birth']!='' && checkdate(iconv_substr($tmp['date_of_birth'],5,2,'utf-8'),iconv_substr($tmp['date_of_birth'],8,2,'utf-8'),iconv_substr($tmp['date_of_birth'],0,4,'utf-8'))) {
			              $date = new DateTime($tmp['date_of_birth']);
			              $now = new DateTime();
			              $interval = $now->diff($date);
			              $age = $interval->y;
			              $data['adm_info']['age'] = $age;
			              $data['adm_info']['req_date_of_birth'] = formatDateThai($tmp['date_of_birth']).' (อายุ '.$age.' ปี)';
						}
						else if($tmp['date_of_birth']!='' && iconv_substr($tmp['date_of_birth'],5,2,'utf-8')!= ''){
							$birthdate = iconv_substr($tmp['date_of_birth'],0,4,'utf-8')+543; 
							$date = new DateTime($tmp['date_of_birth']);
							$now = new DateTime();
							$interval = $now->diff($date);
							$age = $interval->y;
							$data['adm_info']['age'] = $age;
							$data['adm_info']['req_date_of_birth'] = "วัน - เดือน - พ.ศ. ".$birthdate." (อายุ ".$age." ปี)";
						}
						else {
			            	$data['adm_info']['req_date_of_birth'] = ' - ';
			            }
										...
										$data['adm_info']['req_tel_no']					= $tmp['tel_no'];
										$data['adm_info']['req_email_addr'] 			= $tmp['email_addr'];
										$tmp1 = $this->personal_model->getOnce_PersonalAddress($tmp['reg_addr_id']);
										$data['adm_info']['req_reg_add_info'] = $tmp1['addr_home_no'];
										$data['adm_info']['req_reg_add_info'] .= ($tmp1['addr_moo'] != '')?' หมู่'.$tmp1['addr_moo']:'';
										$data['adm_info']['req_reg_add_info'] .= ($tmp1['addr_sub_district'] != '')?' ต.'.$tmp1['addr_sub_district']:'';
										$data['adm_info']['req_reg_add_info'] .= ($tmp1['addr_district'] != '')?' อ.'.$tmp1['addr_district']:'';
										$data['adm_info']['req_reg_add_info'] .= ($tmp1['addr_province'] != '')?' จ.'.$tmp1['addr_province'].' '.$tmp1['addr_zipcode']:'';

										$data['adm_info']['req_reg_add_info'] = ($data['adm_info']['req_reg_add_info']!='')?$data['adm_info']['req_reg_add_info']:'-';
				...
				if($frm->run($this)){//Valid Data
					...
					$data_update_dis = array();
					$data_update_dis = get_inpost_arr('pers_disability');;
					...
					//$row		= $this->welfare_model->get_AdmInfo($adm_info['adm_id']);
					if($data_update['chn_code'] == '007' && $position == '2'  && $org=='174' ){	
						$data_update['insert_user_id'] 	= getUser();
						$data_update['insert_org_id'] 	= $data_update['insert_org_id'];
						$data_update['insert_datetime'] = getDatetime();

					}else{
						unset($data_update['insert_org_id']);
					...
					unset($data_update['req_tel_no']);
					unset($data_update['req_email_addr']);
					unset($data_update['email_addr']);
					...
					$namephoto = $this->files_model->getOnceImg("img",'assets/modules/welfare/uploads');
					 ...
					 $data_update['adm_photo_file'] = 'assets/modules/welfare/uploads/'.$namephoto;
					 $data_update['adm_photo_label'] = $_FILES['img']['name'];
					 $data_update['adm_photo_size'] = $_FILES['img']['size'];
					}

					if($_FILES['img2']['name']!=''){
						$namephoto2 = $this->files_model->getOnceImg("img2",'assets/modules/welfare/uploads');
				   
						$data_update['adm_req_photo_file'] = 'assets/modules/welfare/uploads/'.$namephoto2;
					...
					//update pers_physicality
					$this->common_model->delete_where('pers_physicality','pers_id',get_inpost('adm_info[pers_id]'));
					$data_phy_update								= array();
					$data_phy_update 								= @get_inpost_arr('pers_physicality');
					$this->common_model->insert('pers_physicality',$data_phy_update);

					//update pers_health_record
					$this->common_model->delete_where('pers_health_record','pers_id',get_inpost('adm_info[pers_id]'));
					$data_health_update								= array();
					$data_health_update 							= @get_inpost_arr('pers_health_record');
					$this->common_model->insert('pers_health_record',$data_health_update);

					//update per disability
					$tmp_dis = get_inpost_arr('pers_disability[dis_code]');
					if(!empty($tmp_dis)){
					...
					$row= $this->welfare_model->get_AdmInfo($adm_id);
		  ...
		  /////////////////////////////////////////////////////////////////
					...
					$data['adm_info'] 			= get_inpost_arr('adm_info');
					$data['adm_info']['adm_id'] = $adm_id;
					$data['pers_physicality'] = get_inpost_arr('pers_physicality');
					//$data['adm_info']['adm_id'] = $adm_id;
		...
		/*-- End datepicker custom--*/
  		...
  		/*-- End dropzone style --*/
			...
			if($process_action=='Edit' && get_inpost('bt_submit')=='' && $usrpm['perm_status']=='Yes') {
				...
				//select data and display on adminssion2.php
				$row = $this->welfare_model->getOnce_admInfo($adm_id);
				$data['adm_follow_record'] = $this->welfare_model->getAll_FollowRecord($row['pers_id']);
				...
				if($frm->run($this)){//Valid Data
					...
					$data_follow_update						= get_inpost_arr('adm_follow_record');
					if($data_follow_update['date_of_follow'] != ''){

						$data_follow_update['date_of_follow']	= dateChange($data_follow_update['date_of_follow']);
						$data_follow_update['update_user_id'] 	= getUser();
						$data_follow_update['update_org_id'] 	= get_session('org_id');
						$data_follow_update['update_datetime']	= getDatetime();
						$this->common_model->insert('adm_follow_record',$data_follow_update);
						// adm_info
						$data_update = get_inpost_arr('adm_info');
				...
				$data_update['delete_org_id'] 	= get_session('org_id');
				$data_update['delete_datetime'] = getDatetime();
				$id = $this->input->get('adm_record_id');

				$this->common_model->update('adm_follow_record',$data_update,array('adm_record_id'=>$id));
				redirect('welfare/admission2/Edit/'.$adm_id,'refresh');
			}else {
				page500();
				$this->template->load('index_page',$data,'welfare');
  ...
  public function webblock_list($process_action='Add',$adm_id=0) { //แบบขอรับบริการ (แจ้งเรื่อง)
	...
	public function getDataDead() { 
		...
		$rs = $this->common_model->getWelfareDouble($input['req_id']);
