...
class Admin_model extends CI_Model {
    ...
    public function getOnce_Application($app_id){ //Used for find application 
        return rowArray($this->common_model->custom_query("select * from usrm_application where app_id={$app_id}"));
    }    

    public function chkOnce_usrmPermiss($app_id,$user_id) { //Used for check user permission
    ...
    public function get_KPYProvinceByOrgId($org_id) {
      $query = $this->db->get_where('std_kpy_org_province', array('org_id' => $org_id));
      $row = $query->row_array();
      return $row['province'];
    }

    public function getUserByFingerprintKey($fingerprint_key)
    {
        $sql = "select * ";
        $sql = $sql . " from usrm_user ";
        $sql = $sql . " where user_id = ( ";
        $sql = $sql . "   select user_id ";
        $sql = $sql . "   from usrm_user_fingerprint_authenticated ";
        $sql = $sql . "   where fingerprint_text = '{$fingerprint_key}' limit 1 )";
...
/*    public function getPGP(&$P_ID_arr = array(),$GM_ID = 0,$Parent_ID=0) {
   ...
   public function MapOrgIdToProvinceCode($orgId){
        $proinveCode = "";
        $sql = "select *
                from usrm_org_province
                where org_id ='".$orgId."'";
        $row = rowArray($this->common_model->custom_query($sql));
        if(isset($row)){
            // 2 = พมจ
            if($row['org_parent_id'] == 2){
                $proinveCode = $row['province_code'];
            }
        }
        return $proinveCode;
    }
