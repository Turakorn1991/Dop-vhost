...
$(document).ready(function(){
	$(".btn-save").click(function () {
		...
		} else if ($("input[name='adm_info[pid]']").html() === " - ") {
			toastr.error("โปรดตรวจสอบข้อมูลเลขบัตรประจำตัวประชาชนผู้สูงอายุ", "หน้าต่างแจ้งเตือน");
		} else if ($("input[name='adm_info[date_of_req]']").html() === " - ") {
			toastr.error("โปรดระบุวันที่แจ้งเรื่อง", "หน้าต่างแจ้งเตือน");
		} else if ($("input[name='adm_info[chn_code]']").val() === "") {
	...
	// 		var addr_len = $("#reg_addr").text().trim().split(' ').length;
	// 		$('input[name="pers_addr[addr_zipcode]"]').val($("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-1]);
	// 		$("#select2-Province-container").attr("title",$("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-2].split('.')[1]);
	// 		$("#select2-Province-container").text($("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-2].split('.')[1]);
	// 		$("#select2-Amphur-container").attr("title",$("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-3].split('.')[1]);
	// 		$("#select2-Amphur-container").text($("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-3].split('.')[1]);
	// 		$("#select2-Tambon-container").attr("title",$("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-4].split('.')[1]);
	// 		$("#select2-Tambon-container").text($("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-4].split('.')[1]);
	// 		$('input[name="pers_addr[addr_moo]"]').val($("#reg_addr")
	// 			.text().trim().split(' ')[addr_len-5].split('หมู่')[1]);
	// 		$('input[name="pers_addr[addr_home_no]"]').val($("#reg_addr")
...
//});
// var province = '<?php @$addr_info['district_code']?>';
// $('#Province select').val('10010000');
